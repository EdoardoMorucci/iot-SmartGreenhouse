...
#define SENSOR_TYPE "led_water_actuator"

/* Log configuration */
#include "sys/log.h"
#define LOG_MODULE "Led water node"
#define LOG_LEVEL LOG_LEVEL_APP

PROCESS(led_water_server, "Server for water led");
AUTOSTART_PROCESSES(&led_water_server);
...
extern coap_resource_t res_led_water;
...
PROCESS_THREAD(led_water_server, ev, data)
{
	...
	while(!registered){
		...
		LOG_INFO("[WATER LED] sending registration message\n");	
	...
	LOG_INFO("Starting water led node\n");
	
	coap_activate_resource(&res_led_water, "res_led_water");  
